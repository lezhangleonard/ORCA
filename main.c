...
//    //TimerInit( &TxTimeoutTimer, loraOnTimeoutIrq );
//    //TimerInit( &RxTimeoutTimer, loraOnTimeoutIrq );
//    //TimerInit( &RxTimeoutSyncWord, loraOnTimeoutIrq );
//
////    lora_reset();
//
////    lora_rxchain_calibration();
//
////    lora_set_opmode(RF_OPMODE_SLEEP);
//
...
////    lora_set_modem(radio_registers[i].Modem);
////    lora_write(radio_registers[i].Addr, radio_registers[i].Value);
////  }
////    lora_set_opmode(RF_OPMODE_SLEEP);
////    lora_set_modem();
////    state = RF_IDLE;
...
void lora_set_power(int8_t pa_power)
{
    ...
    paConfig = 0x7F;
    lora_write(REG_LR_PACONFIG, paConfig);
...
void rf_init_lora()
{
    ...
    //    #ifndef REDUCED
    //    lora_set_rxconfig(MODEM_LORA, LORA_BANDWIDTH, LORA_SPREADING_FACTOR,
    ...
    //    // Sync word
    //    lora_write(REG_LR_SYNCWORD, 0x34);
    //    lora_write(REG_LR_FRFMSB, 0xD8);
    //    lora_write(REG_LR_FRFMID, 0xEC);
    //    lora_write(REG_LR_FRFLSB, 0xCC);
    ...
    //    lora_write(REG_LR_INVERTIQ2, RFLR_INVERTIQ2_ON );
    //    lora_write(REG_LR_INVERTIQ, 0x67); // InvertIQ Enabled (bit 6) | default_value (0x27) = 0x67
    //    #ifdef TTN
    //    lora.Settings.LoRaPacketHandler.Size = PHY_PAYLOAD_SIZE;
    //    lora_write(REG_LR_PAYLOADLENGTH, PHY_PAYLOAD_SIZE); // Initializes the payload size
    //    #else
...
int main(void) {
    ...
    uint8_t send_buffer[132] = {0,0,0,0,'H','E','L','L','O'};
...
//    while (1){
//        GPIO_setOutputLowOnPin(VIN_PORT, VIN_PIN);
//
...
//#endif
//void Port_1 (void) {
//    if (GPIO_getInterruptStatus(SLAVE_PORT, SLAVE_IX)){
//        lora_on_dio0irq();
//    }
//    GPIO_clearInterrupt(SLAVE_PORT, SLAVE_IX);     // Clear IRQ
//}
